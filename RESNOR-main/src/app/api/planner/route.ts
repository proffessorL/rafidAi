...
async function fetchStudentDeadlines(userId: string): Promise<{ tasks: DeadlineTask[]; autoDetected: boolean; deadlinesThisWeek: number }> {
  ...
  const quizzes = await db.quiz.findMany({
    where: {
      teacherId: { not: null },
      dueDate: { gte: new Date(now), lte: new Date(oneWeekFromNow) },
  ...
  const tasks: DeadlineTask[] = [
    ...quizzes.filter(q => q.topic).map(q => ({
      id: `quiz-${q.id}`,
      title: q.title,
  ...
  const upcomingCount = tasks.length;

  return {
    tasks,
    autoDetected: upcomingCount >= 3,
    deadlinesThisWeek: tasks.length,
  };
}
...
function buildDeathWeekState(tasks: DeadlineTask[], trigger: 'auto' | 'manual', existingId?: string): DeathWeekState {
  ...
  const isActive = trigger === 'manual' || tasks.some(t => !t.isCompleted);
  return {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { tasks: freshTasks, autoDetected, deadlinesThisWeek } = await fetchStudentDeadlines(userId);
    ...
    if (existingPlan) {
      ...
      if (hasActiveTasks) {
        const progressMap = new Map(
          storedTasks.filter(t => t.isAutoDetected).map(t => [t.id, { hoursSpent: t.hoursSpent, progress: t.progress, isCompleted: t.isCompleted }])
        );
        const mergedTasks = freshTasks.map(ft => {
          const existing = progressMap.get(ft.id);
          return existing ? { ...ft, hoursSpent: existing.hoursSpent, progress: existing.progress, isCompleted: existing.isCompleted } : ft;
        });
        const manualTasks = storedTasks.filter(t => !t.isAutoDetected);
        const allTasks = [...mergedTasks, ...manualTasks];
        const state = buildDeathWeekState(allTasks, trigger, existingPlan.id);

        await db.deathWeekPlan.update({
          where: { id: existingPlan.id },
          data: {
            subjects: JSON.stringify(allTasks),
            goals: JSON.stringify({ totalHoursRemaining: state.totalHoursRemaining, daysUntilLastDeadline: state.daysUntilLastDeadline }),
          },
        }).catch(() => {});

        return NextResponse.json({ success: true, data: state, meta: { autoDetected: trigger === 'auto', deadlinesThisWeek: allTasks.filter(t => !t.isCompleted).length } });
      } else {
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'activate': return handleActivate(request, data);
      case 'deactivate': return handleDeactivate(request);
      case 'update_progress': return handleUpdateProgress(request, data);
      case 'add_task': return handleAddTask(request, data);
      case 'complete_task': return handleCompleteTask(request, data);
      default:
    ...
    console.error('Planner POST error:', message, error);
    return NextResponse.json({ success: false, error: message }, { status: 500 });
  }
}

async function handleActivate(request: NextRequest, data?: Record<string, unknown>) {
  const userId = await resolveUserId(request);
...
async function handleUpdateProgress(request: NextRequest, data?: Record<string, unknown>) {
  const userId = await resolveUserId(request);
...
async function handleAddTask(request: NextRequest, data?: Record<string, unknown>) {
  const userId = await resolveUserId(request);
...
async function handleCompleteTask(request: NextRequest, data?: Record<string, unknown>) {
  const userId = await resolveUserId(request);
